...
def crop_and_pad(image_in, image_out, bbox):
    ...
    """
    if (bbox[2] - bbox[0]) % 2 != 0: # height make even -> odd
        bbox[0] += 1
    if (bbox[3] - bbox[1]) % 2 != 0: # width
        bbox[1] += 1
...
def get_mask_save_segimage(model, image_path,
    ...
    # Detect objects
    r = model.detect([image], verbose=1)[0] # expand dims for batch
...
def user_style_seg(user_input, model, weight, output_dir):
    """
    ...
    return user_fore, user_back, user_mask, user_bbox
...
def image_rendering(tshirt, background, user_bbox, user_mask, output_dir):
    ...
    """
    y1, x1, y2, x2 = user_bbox # top, left, right, bottom
    # roi adjust (rect)
    w = x2 - x1
    h = y2 - y1
    ...
    # cv2.copyMakeBorder(src, top, bottom, left, ri ght, borderType, white)
    ...
    # 이미지 합성
    out = np.where(back==[0,0,0], fore, back).astype(np.uint8) # fore for 0,0,0 pixels, back for non 0
