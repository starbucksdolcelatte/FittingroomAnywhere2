...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(256*1, 256*1, 3),
                 date_time_string_addition='_test', project_dir = '.', image_folder='', to_train=True, to_restore=False, to_one=False):
        # check if training or testing the results
        self.to_train = to_train
        self.to_restore = to_restore
        self.to_one = to_one # generate only one image for demo?
        ...
        if self.to_one:
            data = load_data.load_one(nr_of_channels=self.channels, 
                                    project_dir=self.project_dir, 
                                    subfolder=image_folder)
            self.A_one = data['input_image']
            self.A_one_name = data['input_image_name']
...
#         self.load_model_and_generate_synthetic_images()
        ...
        if self.to_restore:
            load_latest_weights(self.D_A)
            load_latest_weights(self.D_B)
            load_latest_weights(self.G_A2B)
            load_latest_weights(self.G_B2A)
            print("Latest model weights has been restored.")
        ...
        if self.to_train:
            ...
            if self.to_one:
                self.load_model_and_generate_one_fake_images(image_folder)
            else:
                self.load_model_and_generate_synthetic_images()
...
# Save and load
    ...
    def load_latest_weights(self, model): # /saved_models/{self.date_time}/{model.name}_weights_epoch_{epoch}.hdf5
        ...
        latest_epoch = epochs[-1].split('.')[0]

        # path_to_model = os.path.join(latest_model, '{}_model_epoch_{}.json'.format(model.name, latest_epoch))
    ...
    def load_model_and_weights_by_image_folder(self, model, dataset_name='models'):
        path_to_model = os.path.join(self.project_dir, dataset_name, 'model', '{}.json'.format(model.name))
        path_to_weights = os.path.join(self.project_dir, dataset_name, 'model', '{}.hdf5'.format(model.name))
        #model = model_from_json(path_to_model)
    ...
    def load_model_and_generate_one_fake_images(self, dataset_name):

        self.load_model_and_weights_by_image_folder(self.G_A2B, dataset_name)
        fake_B_one = self.G_A2B.predict(self.A_one) # dimension expanded while loading data

        def save_image(image, name, domain):
            if self.channels == 1:
                image = image[:, :, 0]
            toimage(image, cmin=-1, cmax=1).save(os.path.join(
                self.project_dir, dataset_name, 'fake_output')) # deprecated

        for i in range(len(fake_B_one)):
            # Get the name from the image it was conditioned on
            name = self.A_one_name[i].strip('.png') + '_fake.png'
            fake_B = fake_B_one[i]
            save_image(fake_B, name, 'B')

        print('{} fake images have been generated and placed in ./{dataset_name}/fake_output/'
                .format(len(self.A_one)))
